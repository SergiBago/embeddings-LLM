...
def should_ignore_url(url):
    """Check if the URL should be ignored based on file type or language prefix."""
    # Ignore non-markdown types
    for ext in IGNORE_TYPES:
        if url.endswith(ext):
            print(f"Ignoring URL due to extension: {url}")
            return True

    # Ignore if the URL is only a language version without subpages
    for prefix in LANG_PREFIXES:
        if url.endswith(prefix) or prefix + "/" in url:
            print(f"Ignoring URL due to language prefix: {url}")
            return True
...
def scrape_site(url):
    """Main function to scrape a website and convert it to markdown."""
    global pending_urls
    base_url = url
    pending_urls = [base_url]
    
    limit = 50  # Arbitrary limit for safety
    count = 0
    ...
    if not os.listdir(SAVE_FOLDER):
        print("No files were saved in the folder. Check if the URLs are being processed correctly.")
    else:
        print(f"Files saved in {SAVE_FOLDER}: {os.listdir(SAVE_FOLDER)}")
